...
public class MySqlFacultyDAO implements FacultyDAO {
    ...
    public boolean changeAdmissionOpenStatus(String action, Long facultyId) throws SQLException {
        try (Connection con = connection;
             PreparedStatement pstmt = con.prepareStatement("UPDATE  faculty " +
                     "SET admission_open=?" +
                     " WHERE id=?;")) {
    ...
    public FacultyPage findAllSorted(String name_en, String direction, int page, int itemsPerPage) throws SQLException {
        ...
        try (Connection con = connection;
             PreparedStatement pstmt =
                     con.prepareStatement("select f.id, budget_capacity, description_en, description_uk, name_en,name_uk, req_subject1_en,req_subject1_uk, req_subject2_en,req_subject2_uk, req_subject3_en,req_subject3_uk, total_capacity, admission_open," +
    ...
    public void create(Faculty faculty) throws SQLException {
        try (Connection con = connection;
             PreparedStatement pstmt = con.prepareStatement(" INSERT INTO faculty  " +
